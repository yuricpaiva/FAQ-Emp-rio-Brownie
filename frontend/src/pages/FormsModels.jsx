...
function FormsModels() {
  ...
  return <section className="page-stack forms-page">
    ...
    <div className="faq-table-wrap">
      <table className="faq-table forms-models-table">
        ...
        <tbody>
          ...
          {!loading && models.map((model) => <tr className={!model.active ? "is-inactive" : ""} key={model.id}><td><strong>{model.name}</strong><small>{model.description || "Sem descrição"}</small></td><td>{resultTypeLabels[model.resultType]}</td><td>{model.questions.length}</td><td>{model.requiresApproval ? "Com aprovação" : "Direto"}</td><td>{model.defaultObserver?.name || "—"}</td><td><span className={`forms-status forms-status--${model.active ? "active" : "inactive"}`}>{model.active ? "Ativo" : "Inativo"}</span></td><td><div className="faq-table-actions"><button type="button" onClick={() => navigate(`/forms/modelos/${model.id}/editar`)}>Editar</button><button type="button" onClick={() => toggle(model)}>{model.active ? "Inativar" : "Ativar"}</button></div></td></tr>)}
