...
test('Forms API preserves snapshots, permissions, photos, scores and approval states', async (t) => {
  ...
  const payload = {
    ...
    questions: [
      { text: 'Organização original', type: 'SCORE', required: true, photoRequired: true, weight: 1 },
  ...
  const changedPayload = { ...payload, questions: [
    { text: 'Organização alterada', type: 'SCORE', required: true, photoRequired: false, weight: 10 },
  ...
  const adminOnlyModel = await json(await request(base, '/forms/models', { method: 'POST', cookie: adminCookie, body: { name: `Admin only ${suffix}`, active: true, resultType: 'SIMPLE', requiresApproval: false, questions: [{ text: 'Texto obrigatório', type: 'TEXT', required: true, weight: 1 }], permissions: {} } }), 201);
  ...
  const observerPayload = {
    ...
    questions: [{ text: 'Confirmação', type: 'TEXT', required: true, photoRequired: true, weight: 1 }],
    permissions: { fillRoles: ['reader'], fillUserIds: [], approveRoles: ['production_manager'], approveUserIds: [] },
  };
  ...
  assert.equal((await request(base, `/forms/submissions/${defaultObservedDraft.id}/observer`, { method: 'PATCH', cookie: fillerCookie, body: { observerId: flexibleObserver.id } })).status, 409);
  assert.equal((await request(base, `/forms/submissions/${defaultObservedDraft.id}`, { cookie: defaultObserverCookie })).status, 200);
